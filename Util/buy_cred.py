...
def force_stop_purchase():
    ...
    update_gui_element("purchase_status_cred_buy", "Purchase process stopped by user.")
...
def display_preplanning_details(assets_type):
    if dpg.does_item_exist("Buy_Cred"):
        dpg.delete_item("Buy_Cred")

    with dpg.window(label="Buy_Cred", tag="Buy_Cred", width=600, height=400, show=True):
        ...
        dpg.add_button(label="Back", callback=lambda: (dpg.hide_item("Buy_Cred"), dpg.show_item("Treasure Top-Up Menu")))
...
def confirm_assets_purchase():
    ...
    with dpg.window(label="Purchase Confirmation", tag="Purchase Confirmation Window", width=400, height=200):
        dpg.add_text("Your purchase is being processed...", tag="purchase_status_cred_buy")
...
def buy_preplanning_assets():
    ...
    update_gui_element("purchase_status_cred_buy", "Starting continuous purchase...")
    purchase_slot = 1
    ...
    while not purchase_stop_event.is_set():
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                update_gui_element("purchase_status_cred_buy", f"Purchase {purchase_slot} successful. Total balance: {total_balance}")
                ...
                if username and password:
                    ...
                    if access_token:
                        ...
                        if response.status_code == 200:
                            ...
                            update_gui_element("purchase_status_cred_buy", f"Error after re-login: {response.text}")
                    else:
                        logger.error("Re-login failed. Cannot continue purchase.")
                        update_gui_element("purchase_status_cred_buy", "Re-login failed.")
                else:
                    logger.error("No stored credentials found for re-login.")
                    update_gui_element("purchase_status_cred_buy", "No credentials found for re-login.")
            else:
                logger.error(f"Error purchasing item: {response.text}")
                update_gui_element("purchase_status_cred_buy", f"Error: {response.text}")
        except requests.RequestException as e:
            logger.error(f"Network error: {e}")
            update_gui_element("purchase_status_cred_buy", f"Network error: {e}")
    ...
    update_gui_element("purchase_status_cred_buy", "Purchase process stopped.")
    if dpg.does_item_exist("Purchase Confirmation Window"):
        dpg.delete_item("Purchase Confirmation Window")
